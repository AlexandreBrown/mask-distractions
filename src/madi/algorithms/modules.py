...
import madi.algorithms.vit.vit as vit
import math
...
class MaskerNet(nn.Module):
    def __init__(self, obs_shape, frame_stack, masker_num_filters, masker_num_layers, mask_type, mask_threshold, mask_threshold_type):
        super().__init__()
        ...
        self.layers = [
            ...
            nn.Conv2d(obs_shape[0] // num_masks, masker_num_filters, kernel_size=3, stride=1, padding=1, padding_mode='zeros'),
        ]
        for _ in range(2, masker_num_layers):
            self.layers.append(nn.ReLU())
            self.layers.append(nn.Conv2d(masker_num_filters, masker_num_filters, 3, stride=1, padding=1, padding_mode='zeros'))
        self.layers += [nn.ReLU(),
                        nn.Conv2d(masker_num_filters, 1, 3, stride=1, padding=1, padding_mode='zeros'),
                        nn.Sigmoid()]
        self.layers = nn.Sequential(*self.layers)
        ...
        self.threshold_type = mask_threshold_type
