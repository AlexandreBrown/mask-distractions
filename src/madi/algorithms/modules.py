...
class SharedTransformer(nn.Module):
    def __init__(
        ...
        self.out_shape = _get_out_shape_cuda(
            obs_shape, nn.Sequential(self.preprocess, self.transformer)
        )
...
class Critic(nn.Module):
    def __init__(self, encoder, action_shape, hidden_dim):
        super().__init__()
        self.encoder = encoder
        self.Q1 = QFunction(self.encoder.out_dim, action_shape, hidden_dim)
        self.Q2 = QFunction(self.encoder.out_dim, action_shape, hidden_dim)
...
class MaskerNet(nn.Module):
    def __init__(
        ...
        self.layers += [
            nn.ReLU(),
            nn.Conv2d(
                masker_num_filters, 1, 3, stride=1, padding=1, padding_mode="zeros"
            ),
            nn.Sigmoid(),
        ]
        self.layers = nn.Sequential(*self.layers)
        ...
        if self.mask_type == "hard" or self.mask_type == "mixed" and test_env:
            if self.threshold_type == "fix":
                ...
                threshold = torch.kthvalue(
                    mask.view(-1), int((1 - self.threshold) * mask.numel()) + 1
                ).values
