...
import json
import random
import madi.augmentations as augmentations
import subprocess
...
def load_config(key=None):
    ...
    assert (
        dmcgb_datasets is not None
    ), 'DMCGB_DATASETS not set. Use `export DMCGB_DATASETS="/path/to/datasets"`'
...
def save_img(obs, path="obs.png"):
    ...
    torchvision.utils.save_image(obs[0][:3] / 255.0, path)
...
def set_experiment_name(args):
    ...
    dcs_intensity = (
        f"(intens{args.distracting_cs_intensity})"
        if args.distracting_cs_intensity != 0.1
        and args.eval_mode in ["distracting_cs", "all"]
        else ""
    )
    sgqn_quantile = f"_q{args.sgqn_quantile}" if args.algorithm == "sgqn" else ""
    c_weight_decay = (
        f"_cWeightDecay{args.critic_weight_decay}"
        if args.critic_weight_decay != 0
        else ""
    )
    masker_layers = (
        f"_mLay{args.masker_num_layers}"
        if args.algorithm == "madi" and args.masker_num_layers != 3
        else ""
    )
    overlay_alpha = (
        f"_alpha={args.overlay_alpha}"
        if args.overlay_alpha != 0.5 and args.augment == "overlay"
        else ""
    )
    ...
    name = (
        f"{env}_{args.algorithm}{encoder}{svea_alpha}{svea_beta}{aug_type}"
