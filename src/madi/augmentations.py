...
def strong_augment(obs, method, dataset_dir, overlay_alpha=0.5):
    """Alter the observation with a strong augmentation."""
    if method == 'conv':
        obs_aug = random_conv(obs.clone())
    elif method == 'overlay':
        obs_aug = random_overlay(obs.clone(), dataset_dir=dataset_dir, method='default', alpha=overlay_alpha)
    elif method == 'splice':
        obs_aug = random_overlay(obs.clone(), dataset_dir=dataset_dir, method='splice')
    elif method == 'none':
        obs_aug = obs.clone()
...
def random_overlay(x, dataset_dir, method='default', dataset='places365_standard', alpha=0.5, hue=0.0, sat=0.0, val=0.6):
...
def paired_aug(obs, mask):
    ...
    def no_sem(x): return random_overlay(x)
